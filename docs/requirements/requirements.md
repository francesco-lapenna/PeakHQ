...
- **F1.2.6** The user shall be able to have multiple programs saved simultaneously and mark one as active.
...
- **F2.1.1** The system shall integrate with an external food database API to allow searching foods by name.
...
| **Steps** | Manual entry | Total or average daily steps for the week |
...
- **F3.1.1** The user shall be able to log body weight (kg) for any calendar date.
...
- **F3.2.1** The user shall be able to enter or edit the following per week: Lifting (days), Cardio (min), Steps, RHR, VO2Max, Check & Notes.
...
These items need resolution before or during Phase 2 (System Design):

| # | Question | Impact |
|---|---|---|
| OQ-01 | Which external food database API? (Open Food Facts, Edamam, USDA FoodData Central) | Affects API integration design, data availability, and potential cost |
| OQ-02 | Should body weight be entered in kg, lb, or both? | Data model and display |
| OQ-03 | Is "Steps" a weekly total or a daily average? | Affects how the value is entered and displayed |
| OQ-04 | Should multiple programs be runnable simultaneously or just one active program? | Training data model |
| OQ-05 | What is the tech stack? (Frontend framework, backend runtime) | Phase 2 output |
